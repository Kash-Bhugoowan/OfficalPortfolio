"use client";

import { motion, useReducedMotion } from "framer-motion";
import { CASE_STUDY_SECTION_GAP_PX } from "@/lib/motion";
import {
  CASE_STUDY_EYEBROW,
  CASE_STUDY_TITLE,
  CASE_STUDY_BODY,
  CASE_STUDY_SUBTITLE,
  CASE_STUDY_GAP_EYEBROW,
  CASE_STUDY_GAP_TIGHT,
  CASE_STUDY_GAP_CONTENT,
  CASE_STUDY_GAP_BLOCK,
  CASE_STUDY_REVEAL_HIDDEN,
  CASE_STUDY_REVEAL_VISIBLE,
  CASE_STUDY_REVEAL_VIEWPORT,
  CASE_STUDY_REVEAL_TRANSITION,
} from "@/lib/case-studies/styles";

// HSBC-specific "My responsibilities" layout: same eyebrow + 35/65
// title-vs-body row as ChallengeSection/WhatsNextSection, but the title
// column also carries a short supporting line beneath it (per Figma) —
// ChallengeSectionData and WhatsNextSectionData have no room for that,
// so this gets its own (very small) data shape rather than growing
// those shared types unused fields for every other case study.
export type HsbcResponsibilitiesSectionData = {
  eyebrow: string;
  title: string;
  subtitle: string;
  paragraphs: string[];
};

export default function HsbcResponsibilitiesSection({
  data,
}: {
  data: HsbcResponsibilitiesSectionData;
}) {
  const reduceMotion = useReducedMotion();
  const { eyebrow, title, subtitle, paragraphs } = data;

  return (
    <motion.section
      initial={reduceMotion ? undefined : CASE_STUDY_REVEAL_HIDDEN}
      whileInView={reduceMotion ? undefined : CASE_STUDY_REVEAL_VISIBLE}
      viewport={CASE_STUDY_REVEAL_VIEWPORT}
      transition={CASE_STUDY_REVEAL_TRANSITION}
      className="flex flex-col items-center px-6"
      style={{ marginTop: CASE_STUDY_SECTION_GAP_PX }}
    >
      <div className={`mx-auto flex w-full max-w-[1227px] flex-col ${CASE_STUDY_GAP_EYEBROW}`}>
        <span className={CASE_STUDY_EYEBROW}>{eyebrow}</span>
        <div className={`flex flex-col ${CASE_STUDY_GAP_BLOCK} md:flex-row md:items-start`}>
          <div className={`flex flex-col ${CASE_STUDY_GAP_TIGHT} md:w-[35%] md:shrink-0`}>
            <h2 className={CASE_STUDY_TITLE}>{title}</h2>
            <p className={CASE_STUDY_SUBTITLE}>{subtitle}</p>
          </div>
          <div className={`flex flex-col ${CASE_STUDY_GAP_CONTENT} md:flex-1`}>
            {paragraphs.map((paragraph) => (
              <p key={paragraph} className={CASE_STUDY_BODY}>
                {paragraph}
              </p>
            ))}
          </div>
        </div>
      </div>
    </motion.section>
  );
}
